...
export default function ProductAdd() {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      setError("Failed to load categories or attributes");
    }
  };
  ...
  const uploadFiles = async (files) => {
    ...
    try {
      ...
      if (!contentType || !contentType.includes("application/json")) {
        ...
        console.error("Unexpected non-JSON upload response:", text);
      ...
      // ✅ Return only the relative paths (like uploads/filename.jpg)
      return data.paths;
    } catch (err) {
  ...
  const handleProductImageUpload = async (e) => {
    ...
    try {
      ...
      setFormData((prev) => ({
        ...prev,
        images: [...prev.images, ...uploadedPaths], // ✅ Save file paths
      }));
    } catch (err) {
  ...
  const handleVariationImageUpload = async (index, e) => {
    ...
    try {
      ...
      updatedVariations[index].images = [
        ...(updatedVariations[index].images || []),
        ...uploadedPaths, // ✅ Save uploaded file paths
      ];
  ...
  // Attribute logic (select and add values)
  const toggleAttributeSelection = (attrId) => {
    setSelectedAttributes((prev) => {
      const updated = { ...prev };
      if (updated[attrId]) delete updated[attrId];
      else updated[attrId] = [];
      return updated;
    });
  };

  const handleAttributeValueAdd = (attrId, value) => {
    if (!value.trim()) return;
    setSelectedAttributes((prev) => ({
      ...prev,
      [attrId]: [...new Set([...(prev[attrId] || []), value.trim()])],
    }));
  };

  const removeAttributeValue = (attrId, value) => {
    setSelectedAttributes((prev) => ({
      ...prev,
      [attrId]: prev[attrId].filter((v) => v !== value),
    }));
  };

  const generateCombinations = (attrValues) => {
    const keys = Object.keys(attrValues);
    if (!keys.length) return [];

    const build = (index, prefix = {}) => {
      if (index === keys.length) return [prefix];
      const key = keys[index];
      const values = attrValues[key];
      const result = [];
      for (const v of values) {
        result.push(...build(index + 1, { ...prefix, [key]: v }));
      }
      return result;
    };
                      ...
                      {(() => {
                        ...
                        async function createAttribute(name, values) {
                          try {
                            ...
                            if (!res.ok)
                              throw new Error(
                                data.message || "Failed to create attribute"
                              );

                            setAttributes((prev) => [...prev, data.attribute]);
                            setSelectedAttributes((prev) => ({
                              ...prev,
                              [data.attribute._id]: values,
                            }));
                          } catch (err) {
                        ...
                        async function updateAttribute(attrId, name, values) {
                          try {
                            ...
                            // Update attributes state with new data
                            setAttributes((prev) =>
                              prev.map((a) =>
                                a._id === attrId ? data.attribute : a
                              )
                            );
                          } catch (err) {
                        ...
                        async function handleAddAttributeValue(attrId, value) {
                          if (!value.trim()) return;
                          ...
                          const attrToUpdate = attributes.find(
                            (a) => a._id === attrId
                          );
                          if (attrToUpdate) {
                            await updateAttribute(attrId, attrToUpdate.name, [
                        ...
                        async function handleRemoveAttributeValue(attrId, val) {
                          removeAttributeValue(attrId, val);
                          const attrToUpdate = attributes.find(
                            (a) => a._id === attrId
                          );
                          if (attrToUpdate) {
                        ...
                        // Handle existing attribute selection
                        const handleExistingAttributeSelect = (attrId) => {
                          if (!attrId) return;
                          if (selectedAttributes[attrId]) return; // already selected

                          const selectedAttr = attributes.find(
                            (a) => a._id === attrId
                          );
                          const options = selectedAttr?.options || [];
                        ...
                        // Handle removing an attribute entirely
                        const handleRemoveSelectedAttribute = (attrId) => {
                          setSelectedAttributes((prev) => {
                            const updated = { ...prev };
                        ...
                        return (
                          <div className="space-y-6">
                            ...
                            <div>
                              ...
                              <select
                                onChange={(e) =>
                                  handleExistingAttributeSelect(e.target.value)
                                }
                                      ...
                                      >
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                          ...
                                          <input
                                            ...
                                            className="rounded-md border border-slate-300 bg-white px-3 py-1.5 text-sm transition-colors focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-white dark:focus:border-blue-500"
                                          />
                                        </div>
                                      </div>
                                    );
                            ...
                            {/* Add New Attribute */}
                            <div className="rounded-lg border-2 border-dashed border-slate-300 bg-slate-50 p-5 dark:border-slate-600 dark:bg-slate-800/50">
                              ...
                              <div className="space-y-3">
                                <input
                                  ...
                                  id="newAttrName"
                                  className="w-full rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm text-slate-900 placeholder-slate-400 transition-colors focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-white dark:placeholder-slate-500 dark:focus:border-blue-500"
                                />
                                <input
                                  type="text"
                                  placeholder="Values (comma separated, e.g., 256GB, 512GB, 1TB)"
                                  id="newAttrValues"
                                  className="w-full rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm text-slate-900 placeholder-slate-400 transition-colors focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-white dark:placeholder-slate-500 dark:focus:border-blue-500"
                                />
                                <button
                                  ...
                                  className="inline-flex items-center justify-center rounded-lg bg-green-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 dark:focus:ring-offset-slate-900"
                                >
                                  <svg
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          attrKeys.forEach((key) => {
                            const vals = selectedAttributes[key].filter(
                              (v) => v.trim() !== ""
                            );
                          ...
                          const buildCombos = (index, prefix = {}) => {
                            ...
                            const key = keys[index];
                            const results = [];
                            for (const val of validAttrs[key]) {
                              results.push(
                                ...buildCombos(index + 1, {
                                  ...prefix,
                                  [key]: val,
                        ...
                        className="inline-flex items-center justify-center rounded-lg bg-blue-600 px-6 py-3 text-sm font-semibold text-white shadow-lg transition-all hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-slate-900"
                      >
                        <svg
